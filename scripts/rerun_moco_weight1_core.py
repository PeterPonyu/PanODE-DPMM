#!/usr/bin/env python
"""Re-run the core MoCo/DPMM ablation with moco_weight=1.0.

This script retrains the four ablation models used in Figures 2–4 on the
core datasets needed by the current refined figure pipeline:
    - setty
    - dentate
    - lung
    - endo
...
Outputs
-------
- experiments/results/moco_weight1_core/tables/{dataset}_df.csv
    Per-dataset metric tables consumed by Figures 2 and 3.
- experiments/results/moco_weight1_core/models/{model}/{model}_{dataset}_{timestamp}.pt
    Model checkpoints for downstream biological validation.
- experiments/results/moco_weight1_core/models/{model}/{model}_{dataset}_{timestamp}_history.json
    Loss histories for the manually aligned Figure 4 panel (a).
- benchmarks/benchmark_results/crossdata/latents/{dataset}/{model}_{dataset}_{timestamp}.npz
    Latent files consumed by Figures 2 and 4.
- experiments/results/moco_weight1_core/summary.csv
    Flat summary table for quick inspection.

Usage
-----
    python scripts/rerun_moco_weight1_core.py
    python scripts/rerun_moco_weight1_core.py --datasets setty dentate
"""
...
RESULTS_ROOT = ROOT / "experiments" / "results" / "moco_weight1_core"
...
def _result_to_row(model_name: str, result: dict) -> dict:
    row = {"method": SHORT_NAMES.get(model_name, model_name)}
...
def run_dataset(ds_key: str, seed: int, verbose_every: int) -> tuple[list[dict], Path]:
    ...
    print(f"\n{'=' * 72}")
    print(f"Dataset: {ds_key}  |  moco_weight = 1.0  |  models = {MODELS_TO_RUN}")
    print(f"{'=' * 72}")
        ...
        summary_rows.append(
            {
                "dataset": ds_key,
                "model": model_name,
                "method": SHORT_NAMES.get(model_name, model_name),
                "time_s": elapsed,
...
def main():
    parser = argparse.ArgumentParser(description="Re-run the core moco_weight=1.0 ablation")
