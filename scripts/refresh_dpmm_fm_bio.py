#!/usr/bin/env python
"""Refresh biological-validation artifacts from the DPMM-FM core rerun."""
...
from refined_figures.dpmm_shared import DPMM_FM_CORE_MODEL_DIR
...
def latest_checkpoint(dataset: str) -> Path:
    model_dir = DPMM_FM_CORE_MODEL_DIR / SAFE_MODEL
    candidates = sorted(model_dir.glob(f"{SAFE_MODEL}_{dataset}_*.pt"), reverse=True)
    if not candidates:
        raise FileNotFoundError(f"No rerun checkpoint found for {MODEL_NAME} on {dataset} in {model_dir}")
    return candidates[0]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Refresh bio-validation artifacts from the DPMM-FM rerun")
