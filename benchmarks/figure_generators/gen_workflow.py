...
def gen_workflow_png(
    steps: list[dict], out_dir: Path | str, filename: str = "workflow.png", aspect: float = 0.22
) -> str:
    ...
    STEPS: dict[int, list[dict]] = {
        2: [
            ...
            {"label": "Preprocess", "sub": "HVG · norm · log1p", "icon": "preprocess"},
            {
                ...
                "sub": f"3 {model_family} + 3 {baseline_family}",
                "icon": "train",
            },
            {"label": "Evaluate", "sub": f"{n_metrics} metrics × 12 datasets", "icon": "evaluate"},
        ...
        3: [
            ...
            {"label": f"{n_core} Core Metrics", "sub": "per sweep value", "icon": "metrics"},
        ...
        5: [
            ...
            {"label": f"{n_metrics} Metrics", "sub": "per model × dataset", "icon": "metrics"},
            {"label": "Pairwise Scatter", "sub": "convex hulls", "icon": "scatter"},
            {"label": "Trade-off Landscape", "sub": "multi-objective", "icon": "compare"},
        ],
        6: [
            {"label": "3 Repr. Datasets", "sub": "setty · endo · dentate", "icon": "cells"},
            {"label": f"Train 3 {model_family}", "sub": "Base · Trans · Contr", "icon": "train"},
...
def gen_figure_workflow(fig_num: int, out_dir: Path | str, series: str = "dpmm") -> str:
