...
def _check_artist_content_overlap(infos: list[_ArtistInfo], min_overlap_px2: float = 100.0):
    ...
    """
    graphical = [a for a in infos if a.kind in ("collection", "patch", "image", "legend")]
    issues = []
...
def _check_text_vs_artist_overlap(
    infos: list[_ArtistInfo], tol_px: float = 2.0, min_overlap_px2: float = 50.0
):
...
def _check_axes_overflow(infos: list[_ArtistInfo], fig):
    ...
    for ax in fig.get_axes():
        ...
        if not ax_bb:
            continue
        aid = id(ax)
        ax_artists = [a for a in infos if a.ax_id == aid and a.kind in ("collection", "line")]
        for a in ax_artists:
...
def _check_cross_panel_spillover(fig, renderer, tol_px=5.0):
    ...
    for i, (ax_i, bb_i) in enumerate(ax_bboxes):  # noqa: B007
        for child in ax_i.get_children():
            ...
            if isinstance(child, Text):
                ...
                if child_bb is None:
                    continue
                for j, (ax_j, bb_j) in enumerate(ax_bboxes):  # noqa: B007
                    if i == j:
                        continue
                    ...
                    issues.append(
                        {
                            ...
                            "severity": "warning",
                            "detail": (
                                f"Panel label '{txt}' overlaps content '{a.tag}' ({area:.0f} px²)"
                            ),
                            "elements": [txt, a.tag],
                        }
                    )
                ...
                issues.append(
                    {
                        ...
                        "severity": "warning",
                        "detail": (f"Panel labels '{txt}' and '{txt2}' overlap ({area:.0f} px²)"),
                        "elements": [txt, txt2],
                    }
                )
    return issues
            ...
            issues.append(
                {
                    ...
                    "severity": "warning",
                    "detail": (f"Legend in axes extends beyond figure ({', '.join(sides)})"),
                    "elements": ["legend"],
                }
            )
                ...
                issues.append(
                    {
                        ...
                        "severity": "warning",
                        "detail": (
                            f"Legend from axes spills into a neighbouring axes ({area:.0f} px²)"
                        ),
                        "elements": ["legend"],
                    }
                )
...
def _check_legend_vs_other_panel_content(fig, renderer, infos, min_overlap_px2=30.0):
    ...
    data_artists_full = [a for a in infos if a.kind in ("collection", "line", "image", "patch")]
    data_artists_no_patch = [a for a in infos if a.kind in ("collection", "line", "image")]

    for leg_bb, leg_owner_id in legend_infos:
        # Figure-level legends: skip patches (axis backgrounds/spines)
        targets = data_artists_no_patch if leg_owner_id == id(fig) else data_artists_full
        for da in targets:
                ...
                issues.append(
                    {
                        ...
                        "severity": "warning",
                        "detail": (
                            f"Legend overlaps '{da.tag}' in a different panel ({area:.0f} px²)"
                        ),
...
def _check_fig_legend_vs_subplot_content(fig, renderer, infos, min_overlap_px2=30.0):
    ...
    data_artists = [
        a
        for a in infos
        if a.kind in ("collection", "line", "image", "patch") and not any(s in a.tag for s in _SKIP)
    ]
...
def _check_colorbar_internal(fig, renderer, tol_px=1.0):
    ...
    for ax in fig.get_axes():
        is_cbar = hasattr(ax, "_colorbar_info") or getattr(ax, "_colorbar", None) is not None
        if not is_cbar:
            continue
                    ...
                    issues.append(
                        {
                            ...
                            "severity": "warning",
                            "detail": (
                                f"Colorbar tick '{txt_i}' overlaps tick '{txt_j}' ({area:.0f} px²)"
                            ),
                    ...
                    issues.append(
                        {
                            ...
                            "elements": [f"legend_text:{txt_i}", f"legend_text:{txt_j}"],
                        }
                    )
        ...
        if parent is None:
            ...
            for candidate in fig.get_axes():
                if candidate is ax:
                    continue
                c_is_cbar = (
                    hasattr(candidate, "_colorbar_info")
            ...
            issues.append(
                {
                    ...
                    "detail": (
                        ...
                        + (
                            " — auto-fixed (font shrink / relocation)"
                            if fixed
                    ...
                    ax_issues.append(
                        {
                            ...
                            "severity": "warning",
                            "detail": (
                                f"[{title}] Legend occludes '{da.tag}' ({frac:.0%}, {area:.0f} px²)"
                            ),
                        }
                    )
        per_ax[title] = ax_issues
...
def _check_fontsize_adequacy(
    fig, renderer, infos: list[_ArtistInfo], min_pt: float = 6.0, composed_scale: float = 0.5
) -> list[dict]:
...
def detect_all_conflicts(
    ...
    issues.extend(_check_fontsize_adequacy(fig, renderer, infos, min_pt=6.0, composed_scale=0.5))
    ...
    if verbose:
        ...
        if subplot_problems:
            ...
            for title, ax_iss in subplot_problems.items():  # noqa: B007
                for i in ax_iss:
                    print(f"    ⚠ {i['detail']}")
        ...
        if n_warn > 0 or subplot_problems:
            ...
            print(
                f"  ℹ INFO{tag}: 0 warnings, {n_info} info "
...
def detect_conflicts_in_file(png_path: str, label: str = "", verbose: bool = True) -> list[dict]:
    ...
    """
    print(
        f"  NOTE: Conflict detection requires a live Figure object. "
...
def audit_all_figures(series: str = "dpmm"):
    ...
    def _enhanced_check(fig, label="", verbose=True, overlap_tol_px=2):
        ...
        return detect_all_conflicts(
            fig, label=label, verbose=verbose, text_overlap_tol_px=overlap_tol_px
        )
