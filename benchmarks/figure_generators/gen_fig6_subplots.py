...
def gen_heatmap(md, comp_prefix, ds_name, model, out_path, top_genes=30):
    ...
    top_idx_ranked = sorted(top_idx, key=lambda i: -imp_z.max(axis=0)[i])[:top_genes]
    ...
    im = ax.imshow(
        sub_imp, aspect="auto", cmap="RdBu_r", interpolation="nearest", vmin=-vlim, vmax=vlim
    )
    ax.set_yticks(range(sub_imp.shape[0]))
    ax.set_yticklabels(
        [f"{comp_prefix}{k + 1}" for k in range(sub_imp.shape[0])], fontsize=FONTSIZE_TITLE + 1
    )
...
def gen_correlation_heatmap(corr, gene_names, comp_prefix, ds_name, model, out_path, top_genes=30):
    ...
    ax.set_yticklabels(
        [f"{comp_prefix}{k + 1}" for k in range(n_comp)], fontsize=FONTSIZE_TITLE + 1
    )
    ax.set_xticks(range(len(sub_genes)))
    # Truncate long gene names to 8 chars to reduce overlap
    display_genes = [g[:8] for g in sub_genes]
...
def gen_component_umap(umap_emb, latent, labels, comp_prefix, ds_name, model, out_path, n_comp=4):
    ...
    for k in range(K):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_title(f"{comp_prefix}{k + 1}", fontsize=FONTSIZE_TITLE, pad=2, fontweight="normal")
        for sp in ax.spines.values():
            sp.set_linewidth(LINE_WIDTH_SPINE)
    ...
    fig.suptitle(
        f"{short} — {ds_name}  (component intensity)",
        fontsize=FONTSIZE_TITLE,
        y=1.02,
        fontweight="normal",
    )
...
def gen_gene_umap(
    umap_emb, top_gene_expr, top_gene_names, labels, comp_prefix, ds_name, model, out_path, n_comp=4
):
    ...
    fig.suptitle(
        f"{short} — {ds_name}  (top-correlated gene)",
        fontsize=FONTSIZE_TITLE,
        y=1.02,
        fontweight="normal",
    )
...
def gen_enrichment_dotplot(md, comp_prefix, ds_name, model, out_path, top_n=10):
    ...
    sizes = [20 + 120 * (g - mn) / (mx - mn) for g in gcs] if mx > mn else [60] * len(gcs)

    ax.scatter(
        nlps, y_pos, s=sizes, c=comp_colors, edgecolors="white", linewidth=0.4, alpha=0.88, zorder=3
    )
    # Use ytick labels instead of ax.text so tight_layout accounts for them
    ax.set_yticks(y_pos)
    ax.set_yticklabels(labels, fontsize=FONTSIZE_TITLE, color="#333333")
