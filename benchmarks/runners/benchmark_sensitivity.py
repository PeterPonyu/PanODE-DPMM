...
def _trans_cfg(
    latent_dim=LATENT_DIM,
    d_model=TRANS_DMODEL_DEFAULT,
    nhead=TRANS_NHEAD_DEFAULT,
    num_encoder_layers=TRANS_NLAYERS_DEFAULT,
    dropout=TRANS_DROP_DEFAULT,
    warmup_ratio=DPMM_WR_DEFAULT,
):
...
def _cont_cfg(
    latent_dim=LATENT_DIM,
    moco_weight=CONT_MOCO_WEIGHT_DEFAULT,
    moco_temperature=CONT_MOCO_TEMP_DEFAULT,
    dropout=CONT_DROP_DEFAULT,
    warmup_ratio=DPMM_WR_DEFAULT,
):
...
def build_variants():
    ...
    for wr in SWEEPS["warmup_ratio"]["dpmm"]:
        variants.append(
            {
                "name": f"DPMM-Base(wr={wr})",
                "series": "dpmm",
                "sweep": "warmup_ratio",
                "sweep_val": wr,
                "cls": DPMMODEModel,
                "params": _dpmm_cfg(warmup_ratio=wr),
            }
        )
    ...
    for ld in SWEEPS["latent_dim"]["both"]:
        variants.append(
            {
                "name": f"DPMM-Base(dim={ld})",
                "series": "dpmm",
                "sweep": "latent_dim",
                "sweep_val": ld,
                "cls": DPMMODEModel,
                "params": _dpmm_cfg(latent_dim=ld),
            }
        )
        variants.append(
            {
                "name": f"DPMM-Trans(dim={ld})",
                "series": "dpmm",
                "sweep": "latent_dim",
                "sweep_val": ld,
                "cls": DPMMODETransformerModel,
                "params": _trans_cfg(latent_dim=ld),
            }
        )
        variants.append(
            {
                "name": f"DPMM-Cont(dim={ld})",
                "series": "dpmm",
                "sweep": "latent_dim",
                "sweep_val": ld,
                "cls": DPMMODEContrastiveModel,
                "params": _cont_cfg(latent_dim=ld),
            }
        )
    ...
    for dr in SWEEPS["dropout_rate"]["both"]:
        variants.append(
            {
                "name": f"DPMM-Base(drop={dr})",
                "series": "dpmm",
                "sweep": "dropout_rate",
                "sweep_val": dr,
                "cls": DPMMODEModel,
                "params": _dpmm_cfg(dropout=dr),
            }
        )
        variants.append(
            {
                "name": f"DPMM-Trans(drop={dr})",
                "series": "dpmm",
                "sweep": "dropout_rate",
                "sweep_val": dr,
                "cls": DPMMODETransformerModel,
                "params": _trans_cfg(dropout=dr),
            }
        )
        variants.append(
            {
                "name": f"DPMM-Cont(drop={dr})",
                "series": "dpmm",
                "sweep": "dropout_rate",
                "sweep_val": dr,
                "cls": DPMMODEContrastiveModel,
                "params": _cont_cfg(dropout=dr),
            }
        )
    ...
    for dm in SWEEPS["d_model"]["dpmm_trans"]:
        variants.append(
            {
                "name": f"DPMM-Trans(d_model={dm})",
                "series": "dpmm",
                "sweep": "d_model",
                "sweep_val": dm,
                "cls": DPMMODETransformerModel,
                "params": _trans_cfg(d_model=dm),
            }
        )
    ...
    for nh in SWEEPS["nhead"]["dpmm_trans"]:
        variants.append(
            {
                "name": f"DPMM-Trans(nhead={nh})",
                "series": "dpmm",
                "sweep": "nhead",
                "sweep_val": nh,
                "cls": DPMMODETransformerModel,
                "params": _trans_cfg(nhead=nh),
            }
        )
    ...
    for nl in SWEEPS["num_encoder_layers"]["dpmm_trans"]:
        variants.append(
            {
                "name": f"DPMM-Trans(layers={nl})",
                "series": "dpmm",
                "sweep": "num_encoder_layers",
                "sweep_val": nl,
                "cls": DPMMODETransformerModel,
                "params": _trans_cfg(num_encoder_layers=nl),
            }
        )
    ...
    for mw in SWEEPS["moco_weight"]["dpmm_cont"]:
        variants.append(
            {
                "name": f"DPMM-Cont(moco_w={mw})",
                "series": "dpmm",
                "sweep": "moco_weight",
                "sweep_val": mw,
                "cls": DPMMODEContrastiveModel,
                "params": _cont_cfg(moco_weight=mw),
            }
        )
    ...
    for mt in SWEEPS["moco_temperature"]["dpmm_cont"]:
        variants.append(
            {
                "name": f"DPMM-Cont(moco_t={mt})",
                "series": "dpmm",
                "sweep": "moco_temperature",
                "sweep_val": mt,
                "cls": DPMMODEContrastiveModel,
                "params": _cont_cfg(moco_temperature=mt),
            }
        )
...
def main():
    ...
    print(
        f"Defaults — DPMM-Base : wr={DPMM_WR_DEFAULT}, enc={DPMM_EDIMS_DEFAULT}, "
        f"drop={DPMM_DROP_DEFAULT}"
    )
    print(
        f"Defaults — DPMM-Trans: d_model={TRANS_DMODEL_DEFAULT}, nhead={TRANS_NHEAD_DEFAULT}, "
        f"layers={TRANS_NLAYERS_DEFAULT}, drop={TRANS_DROP_DEFAULT}"
    )
    print(
        f"Defaults — DPMM-Cont : moco_w={CONT_MOCO_WEIGHT_DEFAULT}, "
    ...
    for ds_key in ds_keys:
        ...
        for v in variants:
            r = train_variant(v, splitter, device, args.lr, args.epochs, args.verbose_every)
    ...
    for sk, sl in [("dpmm", "DPMM")]:
        ...
        for sw in sdf["Sweep"].unique():
            ...
            if sw_latents:
                ...
                try:
                    plot_umap_grid(sw_latents, last_splitter.labels_test, f"{sl} — {sw}", str(up))
