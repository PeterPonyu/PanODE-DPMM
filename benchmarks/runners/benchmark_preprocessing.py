    ...
    result.update(
        {
            "Series": sk,
            "Sweep": run_cfg["sweep"],
            "SweepVal": run_cfg["sweep_val"],
            "HVG": hvg,
            "MaxCells": nc,
            "ActualCells": n_actual_cells,
            "ActualGenes": n_actual_genes,
        }
    )
...
def main():
    ...
    for ds_key in ds_keys:
        ...
        for run_cfg in runs:
            r = train_variant(
                run_cfg, device, args.verbose_every, args.seed, CACHE_DIR, ds_info["path"]
            )
