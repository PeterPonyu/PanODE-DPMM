...
def parse_args():
    ...
    g.add_argument(
        "--epoch-series", type=str, default="", help="Comma-separated epoch list, e.g. 100,200,300"
    )
    g.add_argument(
        "--lr-series", type=str, default="", help="Comma-separated LR list, e.g. 1e-2,1e-3,1e-4"
    )
    g.add_argument(
        "--series",
        type=str,
        default="all",
        help="all, dpmm, pure, pure-ae, pure-vae or comma-separated",
    )
    g.add_argument(
        "--models", nargs="+", default=None, help="Explicit model names (overrides --series)"
    )
...
def apply_cli_overrides(args):
    ...
    ensure_dirs(
        SERIES_DIRS["dpmm"]["csv"], SERIES_DIRS["dpmm"]["plots"], SERIES_DIRS["dpmm"]["meta"]
    )
...
def run_benchmark_epoch(splitter, adata, epoch, lr, args):
    ...
    for name, cfg in selected_models.items():
        ...
        ep_trained = (
            int(row["EpochsTrained"].values[0])
    ...
    for sk in ("dpmm",):
        ...
        if sdf.empty:
            continue
        sl = {k: v for k, v in latents.items() if paper_group(MODELS[k]["series"]) == sk}
        if sl:
        ...
        try:
            plot_all_metrics_barplot(
                sdf, str(metrics_path), title=f"{display_names[sk]} - All Metrics ({epoch} epochs)"
            )
...
def main():
    ...
    if label_key in adata.obs.columns:
        ...
        print(
            f"  Labels: {len(np.unique(adata.obs['cell_type'].values))} "
    ...
    for lr in lr_series:
        for ep in epoch_series:
            idx += 1
            print(f"\n{'#' * 60}\n# RUN {idx}/{total}: Epochs={ep}, LR={lr:.0e}\n{'#' * 60}")
            df, tag, plots, latents = run_benchmark_epoch(splitter, adata, ep, lr, args)
...
def _print_summary(df, generated_plots):
    ...
    _print_table(
        df,
        "CLUSTERING METRICS",
        cols=[("NMI", ".4f"), ("ARI", ".4f"), ("ASW", ".4f"), ("DAV", ".4f"), ("CAL", ".1f")],
        extra_header=f"{'EpTrained':>10} {'Time':>8}",
        extra_fn=lambda r: f"{int(r.get('EpochsTrained', 0)):>10d} {r.get('Time_s', 0):>7.1f}s",
    )
    ...
    for title, prefix in [
        ...
        _print_table(
            ...
            title,
            cols=[
                (f"{prefix}_distance_correlation", ".4f", "DistCorr"),
    ...
    _print_table(
        ...
        "LSE (Structure)",
        cols=[
            ("LSE_manifold_dimensionality", ".4f", "ManifDim"),
    ...
    _print_table(
        ...
        cols=[
            ("LSE_trajectory_directionality", ".4f", "TrajDir"),
...
def _print_table(df, title, cols, extra_header="", extra_fn=None):
    ...
    for _, r in df.iterrows():
        if "Error" in r and pd.notna(r.get("Error")):
            print(
                f"{str(r.get('Series', ''))[:6]:<6} {r['Model']:<22} "
        ...
        for spec in cols:
            ...
            if pd.notna(val):
                parts.append(f"{int(val):>10d}" if fmt == "d" else f"{val:>10{fmt}}")
            else:
    ...
    for sk, steps in ABLATION_STEPS.items():
        ...
        for model_name, step_label in steps:
            ...
            for key, fmt in ab_metrics:
                ...
                parts.append(f"{val:>10{fmt}}" if pd.notna(val) else f"{'N/A':>10}")
            if prev_row is not None:
                dnmi = row.get("NMI", np.nan) - prev_row.get("NMI", np.nan)
                dari = row.get("ARI", np.nan) - prev_row.get("ARI", np.nan)
                parts.append(f"{dnmi:>10.4f}" if pd.notna(dnmi) else f"{'N/A':>10}")
                parts.append(f"{dari:>10.4f}" if pd.notna(dari) else f"{'N/A':>10}")
            else:
