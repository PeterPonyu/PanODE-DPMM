...
def standardize_labels(adata, label_key):
    ...
    if label_key in adata.obs.columns:
        ...
        print(
            f"  WARNING: Neither '{label_key}' nor 'cell_type' found in obs. "
...
def run_dataset(
    ...
    for mname, minfo in selected_models.items():
        r = train_one(mname, minfo, splitter, ds_info["data_type"], device, verbose_every)
    ...
    meta = {
        ...
        "n_labels": n_labels,
        "epochs_per_model": {
            m: info["params"].get("fit_epochs", EPOCHS) for m, info in selected_models.items()
        },
        "lr": LR,
        "batch_size": BATCH_SIZE,
        "latent_dim": LATENT_DIM,
    ...
    if not no_plots and latents:
        ...
        try:
            ...
            plot_all_metrics_barplot(
                df, str(bp), title=f"Cross-Dataset: {ds_key} ({ds_info['data_type']})"
            )
            print(f"  Barplot: {bp}")
        except Exception as e:
        ...
        try:
            ...
            for series_tag, order in [("dpmm", MODEL_ORDER_DPMM)]:
                ...
                plot_core_metrics_barplot(
                    sdf, str(sp), title=f"{ds_key} ({series_tag.upper()} series)", series=series_tag
                )
...
""",
    ...
    for ds_key in ds_keys:
        ...
        print(f"\n── {ds_key} ({dt}) ──")
        print(
            f"{'Model':<25} {'NMI':>7} {'ARI':>7} {'ASW':>7} {'DAV':>7} "
