...
\caption{\textbf{Hyperparameter sensitivity analysis.} Panel~(a): experimental workflow showing 12~datasets evaluated under systematic hyperparameter sweeps ($>$1200 training runs). Panels~(b)--(j): boxplot grids for ten hyperparameters---warmup ratio, latent dimension, encoder size, dropout rate, learning rate, epochs, batch size, weight decay, and HVG count---each showing NMI, ARI, ASW, DAV, DRE-UMAP, and LSE across their swept ranges. Red boxes mark default configurations; metric variance~$\leq$5\% across stable ranges confirms robustness.}
...
\caption{\textbf{External benchmark.} Panel~(a): experimental workflow showing 11~external baselines evaluated alongside the best DPMM variant under a unified preprocessing pipeline (3k~HVGs, log$_{1}$p normalization) across 12~datasets and 41~metrics. Panels~(b)--(d): metric boxplot grids for proposed models, classical baselines (PCA, ICA, NMF, FA, Truncated-SVD, UMAP, t-SNE), and deep/probabilistic baselines (scVI~\cite{lopez2018deep}, scDHA~\cite{tran2021scdha}, SCALEX~\cite{xiong2022online}, $\beta$-VAE~\cite{higgins2017beta}, CLEAR~\cite{han2022clear}, and others), organized by clustering concordance, DRE-UMAP, DRE-tSNE, LSE intrinsic, DREX, and LSEX families.}
\label{fig:external}
\end{figure}
