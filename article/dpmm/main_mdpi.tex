...
\graphicspath{{figures/submission/}{../../refined_figures/output/dpmm/}{../../benchmarks/paper_figures/dpmm/}{../../experiments/results/dpmm/}}
...
\begin{figure}[H]
\centering
\input{figures/submission/F01_architecture}
\caption{\textbf{Adaptive-prior architecture and operating hypotheses.} Panels~(a)--(c) summarize the shared autoencoder, online DPMM refitting, and conditional flow refinement. Panel~(d) states the three pre-specified operating points: label concordance, compact geometry, and projection fidelity.}
\label{fig:arch}
\end{figure}
...
\begin{figure}[H]
\centering
\input{figures/submission/F02_tradeoff}
\caption{\textbf{Geometry--concordance trade-off.} Panels~(a)--(c) compare label concordance, cluster compactness, and projection fidelity from the saved core summary. Panel~(d) places the three variants on the NMI--ASW plane, showing a Pareto relation rather than universal dominance.}
\label{fig:ablation}
\end{figure}
...
\begin{figure}[H]
\centering
\input{figures/submission/F03_statistics}
\caption{\textbf{Core statistical evidence across 12 datasets.} Panels~(a)--(d) report directional wins/losses, Wilcoxon evidence, Cliff's $\delta$, and the scoped interpretation. Geometry gains (ASW and DAV) occur on all 12 datasets, whereas NMI/ARI losses are directional but not signed-rank significant.}
...
\begin{figure}[H]
\centering
\input{figures/submission/F04_sensitivity}
\caption{\textbf{DPMM-FM sensitivity across eight datasets.} Panels~(a)--(d) report mean and standard deviation across datasets for flow weight versus NMI and DRE-UMAP, and flow-noise scale versus ARI and LSE. Only the two sweeps present in the saved sensitivity export are shown.}
\label{fig:sensitivity}
\end{figure}

\begin{figure}[H]
\centering
\input{figures/submission/F05_training}
\caption{\textbf{Saved DPMM-Base training dynamics on setty.} Panels~(a)--(d) show total and reconstruction loss, activation of the DPMM loss after warmup, and snapshot NMI/ARI/ASW. The figure is generated from the saved 600-epoch history and requires no retraining.}
\label{fig:training}
\end{figure}
...
\begin{figure}[H]
\centering
\input{figures/submission/F06_biological}
\caption{\textbf{Biological exploration on saved representations.} Panels~(a)--(c) show the saved setty, endoderm, and dentate UMAP embeddings. Panel~(d) summarizes the strongest dentate GO term per selected latent component. These panels provide qualitative and program-level grounding, not universal biological validation.}
\label{fig:bio}
\end{figure}

\subsection{External Benchmark}
Figure~\ref{fig:external} is a legibility-focused view of the external benchmark: it shows the fully populated 12-dataset core panel and 11 representative baselines---including scVI~\cite{lopez2018deep}, CellBLAST~\cite{cao2020searching}, SCALEX~\cite{xiong2022online}, $\beta$-VAE~\cite{higgins2017beta}, and the MoCo-style CLEAR baseline---across the full 41-metric panel. The catalogue-level statistics in Table~\ref{tab:dpmm_external_winrate} use the broader 56-dataset screen and all 18 external baselines. Thus, the figure is a selected visualization of the benchmark rather than the source of the full-screen win-rate calculation; the selection keeps only datasets with complete three-variant core coverage and representative methods from each baseline family.

\begin{figure}[H]
\centering
\input{figures/submission/F07_external_selection}
\caption{\textbf{External benchmark scope and selection.} Panels~(a)--(c) report full-screen significant wins, core significant wins, and core win rates from the in-repository canonical summary (DPMM-Base / Pure-AE rows). Panel~(d) distinguishes the 56-dataset, 18-baseline statistical screen from the 12-dataset, 11-baseline selected visualization; the primary per-variant Wilcoxon export is archived with the manuscript.}
\label{fig:external}
\end{figure}

\begin{figure}[H]
\centering
\input{figures/submission/F08_external_provenance}
\caption{\textbf{External evidence decomposition and provenance boundary.} Panels~(a)--(c) summarize the shared win-rate catalogue (Topic-* rows are sister-family context only, not DPMM claims). Panel~(d) records closed provenance for the primary per-variant Wilcoxon export (DPMM-Base: 407 tests, 176 significant wins, core 31/44 with regenerable NMI/ARI/ASW/DAV splits); raw per-dataset score dumps remain outside the repository.}
\label{fig:external-provenance}
\end{figure}
...
DPMM-Base achieves a core win rate of 70.5\%, winning 31 of 44 core-metric comparisons against external baselines, with 176 total significant wins across all metrics in the full 56-dataset, 18-baseline screen. This advantage is concentrated on ASW and DAV; on NMI alone, several external baselines---notably scVI~\cite{lopez2018deep} and CLEAR---match or exceed DPMM-Base on individual datasets, consistent with the geometry--concordance trade-off observed internally. The primary per-variant Wilcoxon export and canonical summary are archived with the manuscript (\texttt{data/per\_variant\_external\_wilcoxon.csv}; re-aggregation 407/176/31/44); residual absence of raw per-dataset score dumps is documented in \texttt{PROVENANCE-GAP.md}.
...
\begin{figure}[H]
\centering
\input{figures/submission/F09_utility_cost}
\caption{\textbf{Downstream utility and computational cost.} Panels~(a)--(d) compare $k$NN accuracy, macro-F1, training time, and parameter count. Geometry gains do not translate to improved supervised discrimination, while online DPMM refitting adds modest time without changing parameter count.}
\label{fig:utility-cost}
\end{figure}
...
\textbf{Sister prior regime.}
A companion simplex / topic-prior study (PanODE-Topic / Topic-FM manuscript) characterizes a complementary \emph{no trade-off} operating regime under a different latent prior. That sister manuscript remains separate; we do not import its win rates or merge claims into a single PanODE SOTA narrative. The present work stays the explicit geometry--concordance trade-off / Pareto characterization for the DPMM prior family.
