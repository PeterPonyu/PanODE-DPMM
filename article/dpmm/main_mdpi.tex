...
\caption{\textbf{External benchmark selected view.} Panel~(a): the fully populated core panel, comprising 11 representative external baselines evaluated alongside the DPMM variants under a unified preprocessing pipeline (3k~HVGs, log$_{1}$p normalization) across 12~datasets and 41~metrics. Panels~(b)--(d): metric boxplot grids for proposed models, classical baselines (PCA, ICA, NMF, FA, Truncated-SVD, UMAP, t-SNE), and deep/probabilistic baselines (scVI~\cite{lopez2018deep}, scDHA~\cite{tran2021scdha}, SCALEX~\cite{xiong2022online}, $\beta$-VAE~\cite{higgins2017beta}, CLEAR, and others), organized by clustering concordance, DRE-UMAP, DRE-tSNE, LSE intrinsic, DREX, and LSEX families. The accompanying win-rate table is computed from the broader 56-dataset, 18-baseline statistical screen.}
\label{fig:external}
\end{figure}
