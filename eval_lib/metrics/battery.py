...
class DataSplitter:
    ...
    def _create_splits(self):
        ...
        train_val_idx, test_idx = train_test_split(
            indices, test_size=self.test_size, random_state=self.random_state, shuffle=True
        )
...
def _safe_drex(metrics, x_high, x_low, k):
    try:
        drex = evaluate_extended_dimensionality_reduction(x_high, x_low, n_neighbors=k)
...
def compute_metrics(
    latent: np.ndarray, labels, *, data_type: str = "cluster", dre_k: int = 15
) -> dict:
...
def compute_latent_diagnostics(latent: np.ndarray, max_samples: int = 2000) -> dict:
    ...
    if latent is None or len(latent) == 0:
        return dict.fromkeys(
            (
                ...
                "latent_var_min",
                "latent_var_max",
                "latent_near_zero_dims",
                "latent_pairwise_dist_mean",
                "latent_pairwise_dist_std",
            ),
            np.nan,
        )
...
def convergence_diagnostics(history: dict, window: int = 50) -> dict:
    ...
    recon_pct = ((recon_final - recon_prev) / max(abs(recon_prev), 1e-8)) * 100
