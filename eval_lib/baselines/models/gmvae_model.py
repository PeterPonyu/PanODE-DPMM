...
DISTRIBUTION_CONFIG = {
    ...
    "pgm": {
        ...
        "encoder_layers": {"Vanilla": PGMVanillaEncoderLayer, "Geo": PGMGeoEncoderLayer},
        "decoder_layers": {"Vanilla": PGMVanillaDecoderLayer, "Geo": PGMGeoDecoderLayer},
...
class GMVAEEncoder(nn.Module):
    ...
    def __init__(
        ...
        if layer_type not in self.config["encoder_layers"]:
            raise ValueError(
                f"Invalid layer_type '{layer_type}' for {distribution}. "
...
class GMVAEModel(BaseModel):
    ...
    def _reshape_for_decoder(self, z):
        ...
        if self.config["sample_shape"] == "flat":
            ...
            if self.distribution == "hw":
                ...
                z_3d = torch.cat(
                    [z_2d, torch.zeros(batch_size, z_2d.size(1), 1, device=z.device)], dim=-1
                )
                return z_3d
    ...
    def forward(
        self, x: torch.Tensor, n_samples: int = 1, beta: float = 1.0, iwae: int = 0, **kwargs
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        z_kl = z
        if (
            self.distribution in ("poincare", "pgm", "learnable_pgm", "hw")
            and n_samples == 1
            and z.dim() >= 2
        ...
        return {"total_loss": total_loss, "recon_loss": recon_loss_sum, "kl_loss": kl_loss_sum}


def create_gmvae_model(
    input_dim: int, latent_dim: int = 10, distribution: str = "euclidean", **kwargs
):
    """
    Create GM-VAE model
    ...
    """
    return GMVAEModel(
        input_dim=input_dim, latent_dim=latent_dim, distribution=distribution, **kwargs
    )
