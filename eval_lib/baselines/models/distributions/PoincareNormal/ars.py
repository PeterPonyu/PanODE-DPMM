...
class ARS:
    ...
    def __init__(
        self, logpdf, grad_logpdf, device, xi, lb=-infty, ub=infty, use_lower=False, ns=50, **fargs
    ):
        """
        ...
        if not (self.hprime[:, 0] > 0).all():
            raise OSError("initial anchor points must span mode of PDF (left)")
        if not (self.hprime[:, self.K - 1] < 0).all():
            raise OSError("initial anchor points must span mode of PDF (right)")
        self.insert()
    ...
    def insert(self, nbnew=0, xnew=None, hnew=None, hprimenew=None):
        ...
        self.z[:, 1 : self.K] = (
            diff(self.h[:, : self.K]) - diff(self.x[:, : self.K] * self.hprime[:, : self.K])
        ...
        self.cs = torch.cat(
            (torch.zeros(self.B, 1).to(self.device), torch.cumsum(self.s, dim=-1)), dim=-1
        )
        self.cu = self.cs[:, -1]
