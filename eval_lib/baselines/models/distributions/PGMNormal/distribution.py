...
class Distribution:
    def __init__(self, means, log_gamma_square, c=-1) -> None:
        ...
        self.base1 = Normal(self.normal_mu, (0.5 * self.normal_logvar).exp())

        self.gamma_a = (-self.log_gamma_square).exp() / (4 * -self.c) + 1
        self.log_gamma_b = -self.normal_logvar - log(4 * -self.c)
        self.base2 = Gamma(self.gamma_a, self.log_gamma_b.exp())
    ...
    def log_prob(self, z):
        ...
        log_prob = -kl / (2 * -self.c * self.log_gamma_square.exp()) + 1.5 * (
            target_logvar - self.log_beta_square
        )
    ...
    def rsample(self, N):
        ...
        sample_logvar = (
            torch._standard_gamma(self.gamma_a[None].expand(sample_shape)).log()
            - self.log_gamma_b[None]
        )
    ...
    def kl_div(self, target_dist):
        kl1 = euclidean_kl_div(
            self.normal_mu, self.normal_logvar, target_dist.normal_mu, target_dist.normal_logvar
        )
        kl2 = gamma_kl_div(
            self.gamma_a, self.log_gamma_b, target_dist.gamma_a, target_dist.log_gamma_b
        )
        return kl1 + kl2
