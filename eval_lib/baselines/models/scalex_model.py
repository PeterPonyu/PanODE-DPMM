...
class DSBatchNorm(nn.Module):
    ...
    def __init__(
        self, num_features: int, n_domains: int = 1, eps: float = 1e-5, momentum: float = 0.1
    ):
        super().__init__()
        ...
        self.bns = nn.ModuleList(
            [nn.BatchNorm1d(num_features, eps=eps, momentum=momentum) for _ in range(n_domains)]
        )
    ...
    def forward(self, x: torch.Tensor, domain_id: int | torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(domain_id, int) or (
            isinstance(domain_id, torch.Tensor) and domain_id.dim() == 0
        ):
...
class SCALEXDecoder(nn.Module):
                ...
                self.output_layers = nn.ModuleList(
                    [
                        nn.ModuleDict(
                            {
                                "mean": nn.Sequential(
                                    nn.Linear(prev_dim, output_dim), nn.Softmax(dim=-1)
                                ),
                                "disp": nn.Sequential(
                                    nn.Linear(prev_dim, output_dim), nn.Softmax(dim=-1)
                                ),
                                "dropout": nn.Sequential(
                                    nn.Linear(prev_dim, output_dim), nn.Sigmoid()
                                ),
                            }
                        )
    ...
    def forward(self, z: torch.Tensor, domain_id: int | torch.Tensor = 0):
        ...
        if self.recon_type in ["zinb", "nb"]:
            ...
            output = {
                key: torch.zeros(batch_size, output_dim, device=z.device)
...
class SCALEXModel(BaseModel):
    ...
    def __init__(
        ...
        self.encoder_net = SCALEXEncoder(input_dim, hidden_dims, latent_dim, n_domains, use_bn)
    ...
    def forward(
        self, x: torch.Tensor, domain_id: int | torch.Tensor = 0, **kwargs
    ...
    def _nb_loss(
        self, x: torch.Tensor, mean: torch.Tensor, disp: torch.Tensor, scale_factor: float = 1.0
    ) -> torch.Tensor:
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        if self.recon_type == "zinb":
            ...
            if scale_factor is None:
                scale_factor = x.sum(1)
            recon_loss = self._nb_loss(x, outputs["mean"], outputs["disp"], scale_factor)
        else:
...
def create_scalex_model(
    input_dim: int, latent_dim: int = 10, n_domains: int = 1, recon_type: str = "mse", **kwargs
):
    """
    Create SCALEX model
