...
def _draw_arrow(
    ax, start, end, color=C_GREY, linewidth=1.2, style="->", connectionstyle="arc3,rad=0", zorder=2
):
...
def _draw_variant_row(
    ax, y_base, variant, prior_name, prior_detail, latent_label, panel_letter=None
):
    ...
    _draw_arrow(ax, (x_end_input, y_base + BH / 2), (x_enc, y_base + BH / 2), color=C_ENC_E)
    ...
    x_end_enc = x_enc + BW
    x_lat = x_end_enc + gap
    _draw_arrow(ax, (x_end_enc, y_base + BH / 2), (x_lat, y_base + BH / 2), color=C_LAT_E)

    # Latent box
    LBW = SBW + 0.12
    _draw_box(ax, (x_lat, y_base), LBW, SBH, latent_label, facecolor=C_LAT, edgecolor=C_LAT_E)
    ...
    _draw_box(
        ax,
        (x_mid_lat - prior_w / 2, prior_y),
        prior_w,
        prior_h,
        f"{prior_name} Prior",
        sublabel=prior_detail,
        facecolor=C_PRIOR,
        edgecolor=C_PRIOR_E,
        linewidth=1.0,
    )
    ...
    x_end_lat = x_lat + LBW
    x_dec = x_end_lat + gap
    _draw_arrow(ax, (x_end_lat, y_base + BH / 2), (x_dec, y_base + BH / 2), color=C_DEC_E)
    ...
    x_end_dec = x_dec + BW
    x_out = x_end_dec + gap
    _draw_arrow(ax, (x_end_dec, y_base + BH / 2), (x_out, y_base + BH / 2), color=C_OUTPUT_E)
    ...
    if variant["extra"]:
        ...
        if "Flow" in extra_name:
            ...
            _draw_arrow(
                ax, (flow_cx, y_base), (flow_cx, ex_y + 0.46), color=C_FLOW_E, linewidth=0.8
            )
...
def generate(series, out_dir):
    ...
    stage_y = -0.42
    stage_h = 1.86
    _draw_stage_bg(ax, (-0.10, stage_y), 0.80, stage_h, "Input", C_INPUT_E, alpha=0.07)
    _draw_stage_bg(ax, (0.74, stage_y), 1.30, stage_h, "Encoder", C_ENC_E, alpha=0.07)
    _draw_stage_bg(ax, (2.08, stage_y), 0.94, stage_h, "Latent + Prior", C_LAT_E, alpha=0.07)
    _draw_stage_bg(ax, (3.06, stage_y), 1.30, stage_h, "Decoder", C_DEC_E, alpha=0.07)
    _draw_stage_bg(ax, (4.40, stage_y), 0.78, stage_h, "Output", C_OUTPUT_E, alpha=0.07)
